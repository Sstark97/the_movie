...
const MovieProvider = ({ children }: Children) => {
  ...
  const { movies, movie, page, totalPages, handleChangePage, loading, error} = useProvideMovies()

  return (
    <Provider value={{ movies, movie, page, totalPages, handleChangePage, loading, error }}>{children}</Provider>
  )
}
